...
// Step 7 helpers.Constants and Conversion helper class
public class S7 {
    ...
    /**
     * todo: не тестировано!
     * обработка большого куска памяти полученного с контроллера для boolean блоков данных
     * из особенностей - pos всегда приходит 0, сдвигать нужно бит, он не может быть меньше 0 и больше 7
     * @param buffer размер буфера чтения
     * @param pos всегда 0
     * @param startBit стартовый бит с которого начинается кусок памяти
     * @param length количество значений в пакете, должен совпасть с размером буфера
     * @return
     */
    public static boolean[] GetBitAtValues(byte[] buffer, int pos, int startBit, int length){

        boolean[] result = new boolean[length];

        int bitCalc = startBit;
        for (int i = 0; i < length; i++) {
            ...
            result[i] = (value & mask[bit]) != 0;

            bitCalc++;
            if (bitCalc < 0) bitCalc = 0;
            if (bitCalc > 7) bitCalc = 7;
    ...
    public static int[] GetIntMultipleRegister(byte[] buffer, int pos, int length){
        int[] packet = new int[length]; //todo: переход на динмическую структуру?

        int next = 0;

        for (int i = 0; i < length; i++) {  //на Int используется два бита, поэтому запрошенную, желаемую длинну пакета нужно умножить на 2, шаг массива 2

            int hi = buffer[pos + next];
            next++;
            int lo = buffer[pos + next] & 0x00FF;
            packet[i] = ((hi << 8) + lo);
            next++;
        }
    ...
    //обрабатывает ответ для чисел из 4 бит (Real и DInt)
    public static int[] get4bitMultipleRegister(byte[] buffer, int pos, int length){
        int[] result = new int[length];
        int next = 0;
        int current = 0;
        for (int i = 0; i < length; i++) {
            current = buffer[pos + next];
            current <<= 8;
            next++;
            current += (buffer[pos + next] & 0x0FF);
            current <<= 8;
            next++;
            current += (buffer[pos + next] & 0x0FF);
            current <<= 8;
    ...
    public static float[] GetFloatAtValues(byte[] buffer, int pos, int length){

        int[] mediator = get4bitMultipleRegister(buffer, pos, length);

        float[] result = new float[length];
        for (int i = 0; i < length; i++) {
            result[i] = Float.intBitsToFloat(mediator[i]);
        }
